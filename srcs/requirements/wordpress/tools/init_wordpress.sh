...
# Directory where WordPress files are stored.
WORDPRESS_DIR="/var/www/html"

# WordPress configuration file. Used to detect if WordPress is already configured. 
# Contains the database connection settings and other configuration options for WordPress.
WP_CONFIG_FILE="$WORDPRESS_DIR/wp-config.php"

# PHP-FPM runtime directory. Contains the PHP-FPM socket file and PID file. 
# The socket file is used for communication between the web server and PHP-FPM, while the 
# PID file contains the process ID of the PHP-FPM master process.
PHP_FPM_RUN_DIR="/run/php"

# PHP-FPM pool configuration directory. Each pool defines a set of PHP-FPM processes that handle 
# requests for a specific website or application.
PHP_FPM_CONFIG_DIR="/etc/php/8.2/fpm/pool.d"

# PHP-FPM pool configuration file generated by this script.
...
# WordPress files must be owned by www-data because PHP-FPM runs as www-data. 
# This allows PHP-FPM to read and write WordPress files as needed, such as when uploading media or updating plugins.
...
# Enter the WordPress directory to execute WP-CLI commands. WP-CLI is a tool for managing WordPress installations.
cd "$WORDPRESS_DIR"
...
if [ -f "$WP_CONFIG_FILE" ]; then
	echo "[WORDPRESS] >> Existing WordPress configuration detected. Skipping installation and reusing persisted files."
else
	...
	wp user create "$WP_USER" "$WP_USER_EMAIL" --user_pass="$WP_USER_PASSWORD" --role="$WP_USER_ROLE" --allow-root || true

	echo "[WORDPRESS] >> WordPress installation completed."
fi

# Changes the ownership of WordPress files to www-data to ensure that PHP-FPM can read and write to them.
# This is important for operations like uploading media, installing plugins, and updating WordPress itself.
echo "[WORDPRESS] >> Updating WordPress file ownership..."
chown -R www-data:www-data "$WORDPRESS_DIR"

# Starts PHP-FPM in the foreground. Command exec replaces the shell process with php-fpm, so that PHP-FPM runs as PID 1 in the container. 
